...
Now for curves! Work for $k$ algebraically closed. Curve is an integral scheme of dimension $1$, proper over $k$, all of whose local rings are regular (so it is nonsingular). In this scenario, we have that it is projective over $k$. We will use curve to mean this.
\\

We use scheme in this chapter to mean integral scheme of dimension $1$ of finite type over $k$. Point will mean closed point, unless we specify "generic point"

Important invariants: genus. Geometric genus is $dim_k \Gamma(X,\omega_X)$. Arithmetic genus is 
\[p_a(X) = (-1)^{\dim X}(\chi(\OO_x) - 1)\]
Or for a curve: $(1-\chi(\OO_X))$. Note that $H^0(\OO_X) = \Gamma(\OO_X)$ corresponds to morphisms to $\A^1$. Composing to $X \to \PP^1$ using that $\A^1 \to \P^1$ is separated, we get a proper map $X \to \P^1$. Its image must be closed and contained in $\A^1$, hence a finite union of points. $X$ is connected, so it must be a point. Thus the section is constant and $H^0(\OO_X) = k$.

Thus, we get that $p_a(X) = \dim H^1(X,\OO_X)$.


\begin{proposition} If $X$ is a curve, then
\[p_a(C) = p_g(X) = \dim_k H^1(X,\OO_X)\]
Thus, in this case, we refer to both as the genus. 
\end{proposition}
\begin{proof} We know $p_a(C) = \dim_k H^1(X,\OO_X)$. The fact that
\[p_g(X) = \dim_k H^0(X,\omega_x) =\dim_k H^1(X,\OO_X)\]
is a consequence of Serre duality.
\end{proof}

Recall that a Weil divisor on a curve looks like $\sum n_i P_i$. The degree is $\sum n_i$. We have line bundle assocated to divisors $\LL(D)$, and this gives an isomorphism with $\textup{Pic}$ for nonsingular curves. ($X$ is noetherian, integral, sep). Regular local rings are UFDs.
\end{comment}
%#######################################################################
%#######################################################################
%#######################################################################
