...
\end{proposition}
\red{Doublecheck what this is}

Given an abelian category and $A$ an object, the functor $\Hom(A, \star)$ is a covariant and left exact functor to abelian groups. The functor $\Hom(\star, A)$ is a contravariant left exact functor. 
...
We say an abelian category has \textit{enough injectives} if every object is a subobject of an injective object. This implies that every object has an injective resolution: you may think of this as basically the meaning of "enough injectives." (Any two injectives are, in that case, homotopy equivalent \red{FILL IN})
...
\begin{construction} Suppose $F: \frak{A} \to \frak{B}$ is a covariant left exact functor. We construct the \textbf{right derived functors} $R^i F$, $i \ge 0$, as follows. For each $A$ of $\frak{A}$, pick an injective resolution. Then define
...
\begin{proposition} Let $i Y \to X$ be a closed subscheme of $X$. Then
\[H^i(Y,\FF) \cong H^i(X, j_*\FF)\]
(Hence, you will sometimes see people write $\FF$ instead of $j_* \FF$).
\end{proposition}
...
\end{enumerate}
\end{theorem}
\red{Maybe say a little motivation for this}
...
\end{remark}
These sheaf cohomology groups have a few properties. (These satisfy the "desired properties" we would want it to have outlined in Chapter 18.1 of Vakil).
...
\item Given a short exact sequence of quasicoherent sheaves on $X$, the right derived functor interpretation means a short exact sequence $0 \to \FF \to \GG \to \HH \to 0$ induces a long exact sequence on cohomology.
...
Then $\pi$ is a closed embedding into projective space, this isomorphism will help us translate between computations on projective schemes and computations on projective space.
\end{enumerate}
\end{proposition}
...
\begin{itemize}
\item The cohomology vanishes in degree above $n$ (corresponding to the fact that $\PP^n_A$ is covered by $n+1$ affines). \red{check}
...
\item The top cohomology group is one-dimensional for $m = -n-1$ and $A = k$. This is an example of Serre duality:
\[\red{do the equation}\]
...
\begin{remark} \red{Something something here about how you can compute with acyclic resolution and not injective}
\end{remark}
