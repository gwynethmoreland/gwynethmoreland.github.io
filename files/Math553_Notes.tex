...
Time to do a relative version of smoothness! \red{April 17 notes from Nolan}
...
\begin{remark} If $X$ is integral, the last condition is equivalent to $\Omega_{X/Y}$ being locally free (on $X$) of rank $n$. (This is by Lemma 8.9 in Hartshorne: if $A$ is a noetherian local integral domain with residue field $k$ and quotient field $K$: $M$ is a finitely generated $A$-module with $\dim_k M \otimes_A k = \dim_K M \otimes_A K = r$ if and only if $M$ is free of rank $r$. This is a Nakayama lemma proof.
\end{remark}

\begin{example} If $Y = \spec k$ with $k$ algebraically closed, $X$ is smooth over $k$ if and only if $X$ is regular and $n$-dimensional. (So if irreducible and separated: $\iff$ nonsingular)
\end{example}
...
\begin{remark} \red{part about integral base} In general, families over non-integral things are a little wacky.
\end{remark}
...
\begin{proposition} Let $f: X \to Y$ be morphism of nonsingular varieties over an algebraically closed field $k$. Let $n = \dim X - \dim Y$. The following are equivalent:
\begin{enumerate}[(i)]
\item $f$ is smooth of relative dimension $n$
\item $\Omega_{X/Y}$ is locally free of rank $n$ on $X$
\item for every closed point $x \in X$, the induced map on Zariski tangent spaces $T_f: T_x \to T_y$ is surjective.
\end{enumerate} 
\end{proposition}

\begin{lemma} Let $f:X \to Y$ be a dominant morphism of integral schemes of finite type over al algebraically closed field $k$ of char $0$. Then there is a nonempty open set $U \subseteq X$ such that $f: U \to Y$ is smooth.
\end{lemma}

\begin{remark} Note that this means \red{something something locally checking}
\end{remark}

\begin{proposition} Some basic properties of smooth morphisms include: \red{finish}
\end{proposition}

\begin{example} A variety over $\overline{k}$ is smooth if and only if it is nonsingular. \red{remind def?}
\end{example}

\begin{theorem} $f: X \to Y$ is smooth of relative dimension $n$ if and only if the following two conditions hold:
\begin{enumerate}
\item $f$ is flat and,
\item For all $y \in Y$, $X_{\overline{y}} = X_y \times_{k(y)} \overline{k(y)}$ is equidimensional of of dimension $n$ and regular (nonsingular). 
\end{enumerate}
Thus, smoothness can be reduced to a check over algebraically closed fields.
\end{theorem}
