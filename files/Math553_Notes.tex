...
For the next topic, we need the rough notion of a group variety. A group variety $G$ over an algebraically closed field is a variety $G$ with multiplication and inverse morphisms
\[\mu: G \times G \to G, \rho: G \to G\]
such that the set of closed points of $G$ becomes a group under the operation $\mu$, and the inverses given by $\rho$.

For example. $\mathbb{G}_m$ is an group scheme: its inverse is given by $t \mapsto t^{-1}$ and its multiplication map is induced by 
\begin{align*}
\spec k[t,t^{-1}] \to \spec k[t,t^{-1}, s, s^{-1}]
t \mapsto t \otimes s
\end{align*}
Then one can show that the prime ideal $(t-a,s-b)$ pulls back to $(t-ab)$.

A group variety $G$ acts on a variety $X$ if there is a morphism $\theta: G \times X \to X$ which induces a homomorphism $G(k) \to \textup{Aut}(X)$ of groups. A homogeneous space is a variety $X$ and group variety $G$ such that the closed points of $G$ act transitively on the closed points on $X$.

The canonical example in peoples' heads:

\begin{remark} Projective space $\PP^n_k$ is a homogeneous space for the action of $G = \textup{PGL}_n$.
\end{remark}

\begin{remark} Homogeneous spaces are nonsingular. It is nonsingular on an open set, but then the transitive group action means nonsingular everywhere.
\end{remark}

\begin{theorem}[Kleiman] We work over an algebraically closed field $k$ of char $0$. Let $X$ be a homogeneous space of a group $G$. Let $f: Y \to X, g: Z \to X$ be morphisms of nonsingular varieties $Y,Z$ to $X$ (often, they are closed embeddings). For any $\sigma \in G(k)$, let $Y^{\sigma}$ be $Y$ with the morphism $\sigma \circ f$ to $X$. (That is, translate the closed subscheme). Then there is a nonempty over subset $V \subseteq G$ such that for every $\sigma in V(k)$
\[Y^{\sigma} \times_X Z\]
is nonsingular and either empty or of dimension exactly
\[\dim Y + \dim Z - \dim X\]
(If $Y,Z$ are closed subschemes, this fiber product comptues the intersection. So, if you have two closed subschemes and take generic translates of one, they intersect smoothly in the expected dimension. If $Y,Z$ are closed subschemes of complimentary dimension, this says taking generic translates of one results in an intersection that is a collection of reduced points. This is very nice for computing \textit{effective} and \textit{nef cones} in Grassmannians).
\end{theorem}

\includegraphics[scale=.35]{hart1.png}\\
\includegraphics[scale=.35]{hart2.png}

\begin{corollary}[Bertini's theorem] Let $X$ be a nonsingular projective variety over an algebraically closed field $k$ of char $0$. Let $L$ be a linear system without base points. Then almost every element of $L$, considered as a closed subscheme of $X$, is nonsingular (may be reducible, though). 
\end{corollary}
\begin{proof}
Let $f: X \to \PP^n$ be the morphism to $\PP^n$ determined by the bpf system. Consider $\PP^n$ as a homogeneous space under the action of PGL. Apply the theorem taking $g: H \to \PP^n$ being the inclusion of a hyperplane. For generic $\sigma \in G(k)$, we get $X \times_{\PP^n} H^{\sigma} = f^{-1}(H^{\sigma})$ is nonsingular. But the divisor $f^{-1}(H^{\sigma})$ are just elements of the linear system, by construction of $f$. Thus almost all (i.e. generically) the elements are nonsingular.
\end{proof}
