...
%#######################################################################
\begin{comment}
\section{April 21: Flat limits, smooth morphisms}
...
\begin{remark} This lemma means that flat limits exist for families over punctured curves. That is, the Hilbert scheme is proper. 
\end{remark}

\begin{example} A simple example: $\spec k[x,t]/(x^2-t) \to \spec k[t] \setminus 0$, which has flat limit $k[x]/(x^2)$.
\end{example}

\begin{example} Consider two skew lines in $\PP^3 = \proj k[x_0,x_1,x_2,x_3]$. Let's see what happens when we collide them at a point. We just care about the behavior near the collision point, so let's work in an affine chart $\spec k[x,y,z]$. Consider one line $y=z=0$ and another line $x=0,z=t$. What happens as $t \to 0$?
\\

We get a bunch of schemes cut out by $I_t = (y,z) \cap (x,z-t) = (xy,xz,(z-t)y,(z-t)z)$. As $t \to 0$, this yields the ideal $I_0 = (xy,xz,zy,z^2)$. This is the $x,y$ axes in the plane $z=0$ \textit{together} with an embedded point at the origin. Again, note that if one "cuts out" the axes by computing the quotient ideal $(xy,xz,yz,z^2):(xy,z)$, you get the ideal of the embedded point $(x,y,z)$.
\\
...
\[\spec k[x,y,z,t]/(xy,yz,(z-t)y,(z-t)z) \to \spec k[t] \setminus 0\]
 and filling in the flat limit. Be careful when computing these limits: if you thought of the ideals as $I_t = (y,z) \cap (x,z-t)$ and tried to set $t=0$, you might think the limit is $I_t = (y,z) \cap (x,z) = (xy,z)$, but this is not correct!
\end{remark}
...
We don't have time for it, but in Hartshorne Example III.9.8.4, he computes this limit when $X_1$ is a twisted cubic and its projection to $\PP^2$ is a nodal cubic curve. Similarly, the flat limit is this nodal cubic with an embedded point. Again, the limit "remembers" that it is a limit of space curves and is scheme-theoretically not contained in a plane. Also, note that this limit makes sense: a plain old cubic planar curve has Hilbert polynomial $3m$, not $3m+1$, which is the Hilbert polynomial of the twisted cubic.
\end{example}

\begin{example}  \red{Hartshorne-Hirschowitz gets a lot out of studying}
Hartshorne-Hirschowitz: paper about studying $H^0(I_X(d))$ for $X =$ disjoint union of lines in $\PP^n$. You want that it's (generically) expected dimension: it turns out it's enough to show that any such union of lines can be degenerated to some $\wt{X}$ such that $H^0(I_X(d))$ has desired dimension. This allows you to compute more easily: specialize some of the lines to lie in quadrics of collide, etc.
\end{example}
\end{comment}
