...
with the higher cohomology groups helping measure how the map on global sections fails to be exact (and want to use these in general for measuring obstructions), and with tools to compute when $H^1(X,\FF') = 0$ so that we get exactness. For example, one way to think of the fact that taking global sections is exact when $X$ is affine and $\FF'$ is quasicoherent is that
\[H^1(X,\FF') = 0\]
for $X$ affine and $\FF'$ quasicoherent (this is a theorem we will see later).
...
\begin{remark}Let $(X,\OO_X)$ be a ringed space. If $A = \Gamma(X,\OO_X)$, then for any $\OO_X$-module $\FF$ we have that the cohomology groups of $\FF$ have an $A$-module structure. Likewise, if $X$ is a scheme over $\spec B$ for some ring $B$, the cohomology groups of $\FF$ have a $B$-module structure.
\end{remark}
...
\begin{lemma}\label{lemma:injective-implies-flasque} If $(X,\OO_X)$ is a ringed space, then any $\OO_X$-module is flasque. (Note that taking $\OO_X = \underline{\Z}$ guarantees existence of injective resolutions, and thus flasque resolutions, for general sheaves of abelian groups on a ringed space).
\end{lemma}
\begin{proof}
...
%#######################################################################
\begin{comment}
\section{April 04: properties and vanishing theorems of sheaf cohomology, \v Cech cohomology}
\begin{theorem} We have
\[H^i(X,\FF) = 0 \quad \quad \textup{for all $i > \dim X$}\]
%(And, $H^i(X,\FF) = 0$ for $i < 0$).
\end{theorem}
\begin{proof}
See Hartshorne Theorem III.2.7. The proof is an induction on dimension. \red{In cases where we can swap sheaf cohomology for Cech, this will become a little more immediate.}
...
\[\varinjlim H^i(X,\FF_{\alpha}) = H^i(X, \varinjlim \FF_{\alpha})\]
\end{proposition}
...
\item $H^i(X, \II) = 0$ for all coherent ideal sheaves $\II$.
\end{enumerate}
\end{theorem}
\begin{proof}\textup{ }
\begin{itemize}
...
The latter term is a skyscraper sheaf $k(P)$ supported at $P$. Exactness can be checked at the stalk level. We get:
...
where $\OO^r_X \to \OO_X$ looks like $(a_1,\dots,a_r) \mapsto \sum f_i a_i$, i.e. it is a multiplication map. We want to show that $H^1(X,\FF)$, so that the map on global sections is surjective. By filtering:
\[\FF = \FF \cap \OO_X^r \supseteq \FF \cap \OO_X^{r-1} \supseteq \dots \supseteq \FF \cap \OO_X \]
Each quotient in the filtration looks like a coherent sheaf of ideals in $\OO_X$ and thus has vanishing $H^1$. Considering each of the quotient sequences and the corresponding long exact sequence, we "climb up" the filtration and get that each $\FF \cap \OO_X^{i}$ has vanishing $H^1$. Thus, $H^1(X,\FF) = 0$. But then 
\[\Gamma(X,\OO^r_X) \to \Gamma(X,\OO_X)\]
...
%\end{remark}
These sheaf cohomology groups have a few properties that are worth pointing out. (These satisfy the "desired properties" we would want it to have outlined in Chapter 18.1 of Vakil).
...
extending $\Gamma(Y,\pi_* \FF) \to \Gamma(X,\FF)$. \red{Leray spectral sequence stuff}
\item If $\pi: X \to Y$ is an affine morphism, and $\FF$ quasicoherent on $X$, then the natural map on cohomology groups is an isomorphism
...
\begin{remark} It is worth noting that in practice, and especially in the scope of this course, you will not have to compute cohomology by writing down an injective resolution and computing the cohomology of the complex. You will usually compute it from long exact sequences, vanishing theorems, and using Cech cohomology (which is much more "concrete" and friendly for computing).
\end{remark}

We will eventually see for $\PP^n_A$

\begin{enumerate}
\item $H^0(\PP^n_A, \OO^{\PP^n_A}(m))$ is a free $A$-module of rank $\binom{n+m}{m}$ if $m \ge 0$.
...
\begin{itemize}
\item The cohomology vanishes in degree above $n$ (consequence of previously mentioned theorem, but also can be derived from the fact that $\PP^n_A$ is covered by $n+1$ affines)
...
\item The top degree cohomology vanishes for $m > -n-1$. This is an example of \textbf{Kodaira vanishing}, which gives us, in this case, that for $\OO(p)$ with $p > 0$, we have that 
\[H^n(X, \OO_{\PP^n_k}(p) \otimes \Omega_{\PP^n/k}) = H^n(X, \OO_{\PP^n_k}(p-n-1)) = 0\]
Here $\textup{char }k = 0$. (Kodaira vanishing gives $H^q(X, \LL \otimes \Omega_{X/k}^{\otimes p}) = 0$ for $p+q > \dim X$ where $\LL$ is ample and $X$ projective over $k$. Above we want $q = \dim X$ and $p = 1$ so the inequality is satisfied).
\item The top cohomology group is one-dimensional for $m = -n-1$ and $A = k$. Namely, observe that for $X = \PP^n_k$ we have
\[h^0(X,\OO_{X}) = h^n(X, \OO_{X}(-n-1) \otimes \OO_X^{\vee}) = h^n(X,\omega_X \otimes \OO_X^{\vee})\]
This is a first example of Serre duality:
\[H^i(X,\mathscr{E}) \cong H^{n-i}(X, \omega_X \otimes \mathscr{E}^{\vee})^*\]
where $\mathscr{E}$ is locally free sheaf of finite rank.
\item The alternating sum
\[\sum (-1)^i h^i(\OO^n,\OO(m)) \]
is a polynomial in $m$ of degree $n$. This is an example of a \textbf{Hilbert polynomial}. \red{Add on}
\end{itemize}
\end{remark}
...
\end{proposition}
\begin{proof} The algebra here can be bashed out fairly straightforwardly: you'll get two paths to removing two elements, and cancelling signs from these two computations.
\end{proof}
...
\begin{theorem} Suppose $X$ is quasicompact and separated. THen $\textup{\v H}(\{U_i\},\FF)$ is independent of the choice of finite cover $\{U_i\}$. \red{could say more}. Hence in this case we can suppress the $\{U_i\}$ notation and refer to the \v Cech cohomology groups as $H^i(X,\FF)$. \red{CHECK IF VAKIL REQUIRES AFFINE}
\end{theorem}

\begin{theorem} Let $X$ be noetherian and separated. Let $\{U_i\}$ be an open affine cover of $X$, ad let $\FF$ be quasi-coherent on $X$. Then for all $p \ge 0$, we have
...
\end{align*}
So we get that $H^1$ is, as a group, $k[u^{\pm 1}]/(u^dq(u^{-1}) - p(u))_{p,q}$. \red{Compute rank?}
\end{example}
\end{comment}
%#######################################################################
%#######################################################################
