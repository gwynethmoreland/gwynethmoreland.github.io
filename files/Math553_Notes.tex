...
We will deal with contravariant delta functors, so just reverse the arrows.
...
\begin{definition} An additive functor $F: \frak{A} \to \frak{B}$ is \textbf{effacable} if, for each object $A$ in $\frak{A}$, there is a monomorphism $u: A \to M$ for some $M$ with $F(u) = 0$. It is coeffaceable if for each $A$ there exists an epimorphism $v: P \to A$ such that $F(v) = 0$.
\end{definition}
...
\red{As with the case of Poincar\'e duality, we crucially establish a perfect pairing.}
...
%###$
\item
Both sides are contravariant $\delta$-functors, indexed by $i$. We have that the $T^0$ are isomorphic by part (b). So it remains to show they are coeffaceable (we need coeffaceable because these are contravariant).

Given $\FF$ coherent, we know that we can write 
\[\EE = \oplus_{i=1}^n \OO(-q) \to \FF \to 0\]
with $q >> 0$. So we have this epimorphism going to our $\FF$. Then 
\[\Ext^i(\EE,\omega) = \oplus_{i=1}^n \Ext^i(\OO(-q),\omega) = \Ext^i(\OO_X,\omega(q)) = H^i(X, \omega(q)) = 0\]
for $i > 0$ and $q$ large. On the other hand, we see 
\[H^{n-i}(X,\EE)^{\vee} = \oplus H^{n-i}(X,\OO(-q))^{\vee} = 0\]
